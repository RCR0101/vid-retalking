...
def create_parser():
    """Create argument parser with TTS support"""
    parser = argparse.ArgumentParser(description='Video Retalking with Qwen TTS Integration')
    ...
    parser.add_argument('--outfile', type=str, help='Video path to save result', default='results/output_with_tts.mp4')
    ...
    parser.add_argument('--enhanced_transformer', action='store_true', help='Use enhanced transformer')
...
def main():    
    ...
    if args.audio and args.text:
        raise ValueError("Provide either --audio or --text, not both")
    ...
    gen = datagen(imgs_enhanced.copy(), mel_chunks, full_frames, None, (oy1,oy2,ox1,ox2), args)
    ...
    print(f'[Success] Output saved to: {args.outfile}')
    
    # Cleanup TTS audio file if it was generated
    if args.text and os.path.exists(args.audio) and 'tts_audio' in args.audio:
        os.remove(args.audio)
        print('[Cleanup] Temporary TTS audio file removed')


def datagen(frames, mels, full_frames, frames_pil, cox, args):
